...
import {
  ...
  Badge, IBox, SectionHeader, BrokersSection,
} from "../shared";
...
function PlanCard({ plan, onAccessPlatform }: { plan: Plan; onAccessPlatform: () => void }) {
  return (
    ...
    >
      ...
      <button onClick={onAccessPlatform}
...
export default function Planos({ onAccessPlatform }: { onAccessPlatform: () => void }) {
  return (
    <>
      ...
      <section id="planos" className="py-16">
        <div className="max-w-7xl mx-auto px-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-5 gap-4 pt-4">
            {PLANS.map((p) => <PlanCard key={p.name} plan={p} onAccessPlatform={onAccessPlatform} />)}
          </div>
