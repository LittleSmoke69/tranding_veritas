    ...
    <nav className="sticky top-0 z-50 transition-all duration-300"
      ...
      <div className="max-w-7xl mx-auto px-6 flex items-center justify-between h-16">
        ...
        <div className="hidden md:flex items-center gap-5">
          {links.map((l) => (
            <button key={l.view} onClick={() => go(l.view)}
              style={{ color: view === l.view ? FG : MFG, fontSize: "0.9375rem", fontWeight: 600, letterSpacing: "0.01em" }}
        ...
        <div className="hidden md:flex items-center gap-3">
          <button onClick={onAccessPlatform} className="px-5 py-2.5 rounded-xl hover:opacity-90"
      ...
      {open && (
        <div className="md:hidden px-6 pb-4" style={{ background: "rgba(4,5,12,0.98)" }}>
          ...
          <button onClick={onAccessPlatform} className="mt-4 w-full py-3 rounded-xl"
            style={{ backgroundImage: "linear-gradient(to right, #2e64ff, #1f4ee6)", color: "#fff", fontSize: "0.875rem", fontWeight: 700 }}>Acesso à Plataforma</button>
        </div>
      )}
    </nav>
  );
...
function Hero({ onAccessPlatform }: { onAccessPlatform: () => void }) {
        ...
        <div className="grid lg:grid-cols-2 gap-14 items-center">
          <div>
            ...
            <h1 className="mb-7" key={slide}
              style={{ fontFamily: "'Manrope', sans-serif", fontSize: "clamp(2.1rem, 7.5vw, 4.5rem)", fontWeight: 800, lineHeight: 1.06, letterSpacing: "-0.03em", minHeight: "3.2em", animation: "vl-hero-fade 500ms ease", overflowWrap: "break-word" }}>
              <span style={{ color: FG }}>{s.plain} </span>
              <span className="blue-gradient">{s.gradient}</span>
            </h1>

            {/* Subheadline */}
            <p className="mb-10" key={`sub-${slide}`} style={{ fontSize: "1.0625rem", fontWeight: 500, lineHeight: 1.75, color: MFG, maxWidth: "46ch", minHeight: "3.5em", animation: "vl-hero-fade 500ms ease" }}>
              {s.sub}
            </p>

            {/* CTAs */}
            <div className="flex flex-wrap items-center gap-4 mb-8">
              <button onClick={onAccessPlatform} className="flex items-center gap-2.5 px-7 py-4 rounded-xl hover:opacity-90 transition-all hover:scale-105"
            ...
            <div className="flex items-center gap-2 mb-8">
              {HERO_SLIDES.map((_, i) => (
                <button key={i} onClick={() => setSlide(i)} aria-label={`Slide ${i + 1}`}
                  ...
                  {i === slide && <span className="block h-full rounded-full" style={{ background: B, transformOrigin: "left", animation: "vl-hero-progress 5s linear" }} />}
                </button>
              ))}
            </div>
...
// ── Profit Calculator ─────────────────────────────────────────────────────────
function Calculator({ onNavigate }: { onNavigate: (v: LandingView) => void }) {
      ...
      <div className="max-w-5xl mx-auto px-6">
        ...
        <div className="grid lg:grid-cols-2 gap-10">
          ...
          <div>
            ...
            <button onClick={() => onNavigate("planos")} className="w-full py-4 rounded-xl hover:opacity-90"
...
// ── Home ──────────────────────────────────────────────────────────────────────
function Home({ onAccessPlatform, onNavigate }: { onAccessPlatform: () => void; onNavigate: (v: LandingView) => void }) {
  return (
    <>
      <Hero onAccessPlatform={onAccessPlatform} />
      <Stats />
      <MissionVision />
      <Solutions />
      <LiveOps />
      ...
      <BrokersSection />
      <Education />
      <Calculator onNavigate={onNavigate} />
      <ValuesSection />
      <Testimonials />
      <LeadCapture />
      <FAQ />
...
// ── App ───────────────────────────────────────────────────────────────────────
export function LandingPage({ onAccessPlatform }: { onAccessPlatform: () => void }) {
  useLenis();
  const [view, setView] = useState<LandingView>("home");
  return (
    <div className="veritas-landing" style={{ minHeight: "100%" }}>
      <NoiseOverlay />
      <Ticker />
